...
export default function Home() {
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans selection:bg-purple-500 selection:text-white scroll-smooth">
      ...
      {/* Hero Section (코드로 구현한 스마트 커튼월 빌딩 배경) */}
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden border-b border-gray-800 flex items-center justify-center min-h-[80vh]">
        <div className="absolute inset-0 z-0 bg-gray-950 overflow-hidden">
          {/* CSS 추상화 건축물 및 그리드 효과 */}
          <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'linear-gradient(rgba(168,85,247,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(168,85,247,0.4) 1px, transparent 1px)', backgroundSize: '100px 100px', transform: 'perspective(500px) rotateX(60deg) translateY(-100px) translateZ(-200px)' }}></div>
          <div className="absolute left-1/2 bottom-0 w-full max-w-4xl h-[60%] -translate-x-1/2 border-t border-x border-purple-500/30 bg-gradient-to-t from-purple-900/20 to-transparent shadow-[0_-30px_100px_rgba(168,85,247,0.15)] flex">
            {/* 빌딩 창문 패턴 */}
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="flex-1 border-r border-purple-500/10 h-full"></div>
            ))}
          </div>
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,transparent_0%,#030712_80%)]"></div>
        </div>

        <div className="relative z-10 px-6 max-w-5xl mx-auto text-center">
          <div className="inline-block mb-6 px-4 py-1.5 rounded-full bg-purple-900/40 border border-purple-500/50 text-purple-200 text-sm font-bold tracking-wide backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            ...
            <a href="#structure" className="border border-gray-500 text-white px-8 py-3 rounded-full font-semibold hover:bg-gray-800 transition backdrop-blur-sm bg-gray-900/30">
              12-Layer 아키텍처
            </a>
          </div>
        </div>
      ...
      <section id="products" className="px-6 py-24 bg-gray-900">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            {/* BSU-500A */}
            <div className="bg-gray-950 rounded-3xl border border-gray-800 hover:border-blue-500 transition duration-300 overflow-hidden group flex flex-col">
              {/* 코드로 그린 BSU-500A 모듈 이미지 */}
              <div className="h-48 relative bg-gradient-to-br from-blue-950 to-gray-900 p-6 flex justify-center items-center">
                <div className="w-4/5 h-full border border-blue-500/40 grid grid-cols-4 grid-rows-6 gap-1 p-1 bg-gray-950 rounded-sm transform group-hover:scale-105 transition duration-500 shadow-xl">
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="bg-blue-800/30 border border-blue-400/20 relative overflow-hidden">
                      <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-transparent"></div>
                    </div>
                  ))}
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-gray-950 to-transparent z-10 pointer-events-none"></div>
              </div>

              <div className="p-8 flex-1">
                <h3 className="text-2xl font-bold text-gray-100 mb-1">BSU-500A</h3>
                <p className="text-blue-400 text-sm font-semibold mb-4">A-Series : 통합 플랫폼</p>
                <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                  FAPbI₃ 단일 페로브스카이트 상부셀. 집광, 분광, 열저장이 하나의 유닛에서 완결되는 고효율 BIPV.
                </p>
                <ul className="space-y-3 mt-auto text-sm text-gray-300">
            ...
            {/* BSU-500B */}
            <div className="bg-gray-950 rounded-3xl border border-gray-800 hover:border-emerald-500 transition duration-300 overflow-hidden group flex flex-col relative">
              {/* 코드로 그린 BSU-500B 모듈 이미지 */}
              <div className="h-48 relative bg-gradient-to-br from-emerald-950 to-gray-900 p-6 flex justify-center items-center">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.15)_0%,transparent_60%)]"></div>
                <div className="w-4/5 h-full border border-emerald-500/50 grid grid-cols-4 grid-rows-6 gap-1 p-1 bg-gray-950 rounded-sm transform group-hover:scale-105 transition duration-500 shadow-[0_0_20px_rgba(16,185,129,0.2)] z-10">
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="bg-emerald-800/40 border border-emerald-400/30 relative overflow-hidden flex items-center justify-center">
                      <div className="w-full h-px bg-emerald-400/20"></div>
                      <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/10 to-transparent"></div>
                    </div>
                  ))}
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-gray-950 to-transparent z-20 pointer-events-none"></div>
              </div>

              <div className="p-8 flex-1 relative z-30">
                <h3 className="text-2xl font-bold text-gray-100 mb-1">BSU-500B</h3>
                <p className="text-emerald-400 text-sm font-semibold mb-4">B-Series : 트리플 양이온</p>
                <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                  Drop-in 업그레이드를 통해 단일 조성의 열화 한계를 극복하고 발전량과 신뢰성을 극대화.
                </p>
                <ul className="space-y-3 mt-auto text-sm text-gray-300">
                  <li className="flex justify-between border-b border-gray-800 pb-2">
                    <span className="text-gray-500">셀 구조</span> <span className="text-emerald-400 font-bold">Cs/FA/MA 탠덤</span>
                  </li>
                  <li className="flex justify-between border-b border-gray-800 pb-2">
                    <span className="text-gray-500">출력 (보증)</span> <span className="text-emerald-400 font-bold">450W (집광 580W)</span>
                  </li>
                  <li className="flex justify-between border-b border-gray-800 pb-2">
            ...
            <div className="bg-gradient-to-b from-gray-900 to-purple-950/50 rounded-3xl border border-purple-500/50 shadow-[0_0_30px_rgba(168,85,247,0.2)] group transform lg:-translate-y-4 overflow-hidden flex flex-col relative">
              <div className="absolute top-0 right-0 bg-purple-500 text-white text-xs font-black px-4 py-1.5 rounded-bl-2xl z-30 shadow-lg">
                AUTONOMOUS FLAGSHIP
              </div>
              
              {/* 코드로 그린 BSU-600A 모듈 이미지 (AI 칩셋 탑재) */}
              <div className="h-52 relative bg-gradient-to-br from-purple-950 to-gray-900 p-6 flex justify-center items-center overflow-hidden">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(168,85,247,0.3)_0%,transparent_70%)] animate-pulse"></div>
                <div className="w-4/5 h-[110%] border-2 border-purple-500/60 grid grid-cols-6 grid-rows-8 gap-0.5 p-1 bg-gray-950 rounded-md transform perspective-[800px] rotateX(10deg) group-hover:scale-105 transition duration-500 shadow-[0_0_40px_rgba(168,85,247,0.3)] z-10 relative">
                  {Array.from({ length: 48 }).map((_, i) => (
                    <div key={i} className="bg-purple-800/40 border border-purple-400/20"></div>
                  ))}
                  {/* 중앙 NPU AI 칩셋 표현 */}
                  <div className="absolute inset-0 m-auto w-16 h-16 bg-gray-950 border border-purple-400 flex items-center justify-center rounded shadow-[0_0_20px_#a855f7] z-20">
                    <span className="text-[10px] font-black text-purple-200 tracking-wider">NPU</span>
                  </div>
                  {/* 글래스 반사 효과 */}
                  <div className="absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-transparent z-30"></div>
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-transparent to-transparent z-20 pointer-events-none"></div>
              </div>

              <div className="p-8 flex-1 relative z-30">
                <h3 className="text-3xl font-bold text-white mb-1">BSU-600A</h3>
                <p className="text-purple-400 text-sm font-bold mb-4">Autonomous Edition</p>
                ...
                <ul className="space-y-3 mt-auto text-sm text-gray-300">
                  <li className="flex justify-between border-b border-purple-900/50 pb-2">
                    <span className="text-purple-400/70 font-semibold">셀 / 광학</span> <span className="text-white font-bold">4단 탠덤 + QLSC</span>
                  </li>
                  <li className="flex justify-between border-b border-purple-900/50 pb-2">
      ...
      <section id="specs" className="px-4 py-24 bg-gray-950 border-y border-gray-800">
